...
static void walktree_ctx(struct walk_context *ctx) {
  ...
  while (de = readdir(dir)) {
    ...
    if (S_ISDIR(ctx->st.st_mode)) {
      ...
      subdir.dir_fd =
          openat(ctx->dir_fd, ctx->name, O_RDONLY | O_DIRECTORY | O_NOFOLLOW);
      if (subdir.dir_fd < 0)
...
static void walktree(struct walk_context *ctx, const char *path,
                     void (*callback)(struct walk_context *ctx)) {
...
static void rmtree_helper(struct walk_context *ctx) {
  if (ctx->is_dir) {
...
static void chowntree_helper(struct walk_context *ctx) {
  if (S_ISREG(ctx->st.st_mode) || S_ISDIR(ctx->st.st_mode) ||
      ctx->keep_special_files) {
    if (fchownat(ctx->dir_fd, ctx->name, ctx->chown_uid, ctx->chown_gid,
...
// TODO: maybe is to recursive chown folder?
